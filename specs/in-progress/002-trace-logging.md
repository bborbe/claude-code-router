...
| `trace` key present but non-boolean (e.g. `trace: "yes"`) | `yaml.Unmarshal` into `Config.Trace bool` applies YAML bool coercion rules; a non-coercible value fails `Load` with the existing parse error path | Operator corrects the YAML; restart |
