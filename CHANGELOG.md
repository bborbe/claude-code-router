...
- **fix: disable HTTP server `WriteTimeout` and `ReadTimeout` (were 30s each).** libhttp.NewServer's defaults capped each leg of the streaming chain — `WriteTimeout=30s` killed any SSE response that streamed for more than 30 seconds (router → claude); `ReadTimeout=30s` killed any request body that took more than 30 seconds to upload (claude → router, relevant for `/compact`'s large session context). Both are wrong defaults for an LLM-proxy use case. Anthropic's per-request budget + claude-code's client-side timeouts are the real caps. `ReadHeaderTimeout=10s` and `IdleTimeout=60s` stay at defaults (those cap pre-body header reads and idle-keepalive recycling, both safe at single-digit seconds for streaming).
